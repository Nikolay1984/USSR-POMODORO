...
(function ($ , window , undefined) {
	...
	function CreateRoundSlider(n , t) {
		for (var i , r , u = 0; u < this.length; u++) {
			if (i = this[u] , r = $data(i , pluginName) , r) {
				if ($isPlainObject(n)) {
					typeof r.option === "function" ? r.option(n) : i.id && window[i.id] && typeof window[i.id].option === "function" && window[i.id].option(n);
				} else if (typeof n === "string" && typeof r[n] === "function") {
					if ((n == "option" || n.startsWith("get")) && t[2] === undefined) {
						return r[n](t[1]);
					}
					r[n](t[1] , t[2]);
				}
			} else {
				$data(i , pluginName , new RoundSlider(i , n));
			}
		}
		return this;
	}
	...
	$.fn[pluginName] = function (n) {return CreateRoundSlider.call(this , n , arguments);};
	RoundSlider.prototype = {
		...
		_initialize: function () {(this._isBrowserSupport = this._isBrowserSupported() , this._isBrowserSupport) && (this._originalObj = this.control.clone() , this._isReadOnly = !1 , this._checkDataType() , this._refreshCircleShape());} ,
		_render: function () {
			if (this.container = createElement("div.rs-container") , this.innerContainer = createElement("div.rs-inner-container") , this.block = createElement("div.rs-block rs-outer rs-border") , this.container.append(this.innerContainer.append(this.block)) , this.control.addClass("rs-control").empty().append(this.container) , this._setRadius() , this._isBrowserSupport) {
				this._createLayers() , this._setProperties() , this._setValue() , this._bindControlEvents("_bind") , this._checkIE();
			} else {
		...
		_createLayers: function () {
			var i = this.options.width , t = this._start , n;
			...
			this.lastBlock = createElement("span.rs-block").css({padding: i});
		...
		_appendTooltip: function () {this.container.children(".rs-tooltip").length === 0 && (this.tooltip = createElement("span.rs-tooltip rs-tooltip-text") , this.container.append(this.tooltip) , this._tooltipEditable() , this._updateTooltip());} ,
		_removeTooltip: function () {this.container.children(".rs-tooltip").length != 0 && this.tooltip && this.tooltip.remove();} ,
		...
		_setAnimation: function () {this.options.animation ? this._addAnimation() : this._removeAnimation();} ,
		...
		_setRadius: function () {
			var t = this.options.radius , i = t * 2 , n = this.options.circleShape , r = i , u = i , f , e;
			if (this.container.removeClass().addClass("rs-container") , n.indexOf("half") === 0) {
				...
				n.indexOf("quarter") === 0 ? (r = u = t , f = n.split("-") , this.container.addClass(f[0] + " " + f[1] + " " + f[2])) : this.container.addClass("full " + n);
			}
			e = {height: r , width: u};
			this.control.css(e);
			this.container.css(e);
		} ,
		_border: function () {return parseFloat(this.block.css("border-top-width")) * 2;} ,
		...
		_createHandle: function (n) {
			var t = createElement("div.rs-handle rs-move") , i;
			t.attr({index: n , tabIndex: "0"});
			var r = this.control[0].id , r = r ? r + "_" : "" ,
		...
		_refreshHandle: function () {
			var hSize = this.options.handleSize , h , w , isSquare = !0 , s , diff;
			if (typeof hSize === "string" && isNumber(hSize)) {
				if (hSize.charAt(0) === "+" || hSize.charAt(0) === "-") {
					...
					hSize.indexOf(",") && (s = hSize.split(",") , isNumber(s[0]) && isNumber(s[1]) && (w = parseFloat(s[0]) , h = parseFloat(s[1]) , isSquare = !1));
				}
			}
		...
		_handleDefaults: function () {return {angle: this._valueToAngle(this.options.min) , value: this.options.min};} ,
		_handles: function () {return this.container.children("div.rs-bar").find(".rs-handle");} ,
		_activeHandleBar: function () {return $(this.container.children("div.rs-bar")[this._active - 1]);} ,
		...
		_elementDown: function (n) {
			var u , t , r , h , i;
			if (!this._isReadOnly) {
				if (u = $(n.target) , u.hasClass("rs-handle")) {
					this._handleDown(n);
				} else {
					var f = this._getXY(n) , e = this._getCenterPoint() , o = getdistance(f , e) ,
						s = this.block.outerWidth() / 2 , c = s - (this.options.width + this._border());
		...
		_handleMove: function (n) {
			n.preventDefault();
			var u = this._getXY(n) , f = this._getCenterPoint() , t = this._getAngleValue(u , f) , i , r;
			i = t.angle;
			r = t.value;
		...
		_handleKeyUp: function () {this._addAnimation();} ,
		...
		_getKeyValue: function (n) {return this._rangeSlider ? n == "Home" ? this._active == 1 ? this.options.min : this._handle1.value : this._active == 1 ? this._handle2.value : this.options.max : n == "Home" ? this.options.min : this.options.max;} ,
		_elementScroll: function (n) {
			if (!this._isReadOnly) {
				n.preventDefault();
				var i = n.originalEvent || n , r , t , f , u;
		...
		_changeSliderValue: function (n , t) {
			var r = this._oriAngle(t) , i = this._limitAngle(t);
			if (this._rangeSlider || this._showRange) {
				if (this._active == 1 && r <= this._oriAngle(this._handle2.angle) || this._active == 2 && r >= this._oriAngle(this._handle1.angle)) {
					...
					this.bar.rsRotate(i);
					this._updateARIA(n);
					var e = this._oriAngle(this._handle2.angle) - this._oriAngle(this._handle1.angle) , u = "1" ,
						f = "0";
					e <= 180 && (u = "0" , f = "1");
		...
		_updateARIA: function (n) {
			...
			this.options.sliderType == "range" ? (t = this._handles() , t.eq(0).attr({"aria-valuemin": i}) , t.eq(1).attr({"aria-valuemax": r}) , this._active == 1 ? t.eq(1).attr({"aria-valuemin": n}) : t.eq(0).attr({"aria-valuemax": n})) : this.bar.children().attr({
				"aria-valuemin": i ,
				"aria-valuemax": r
			});
		} ,
		...
		_getCenterPoint: function () {
			...
			return {x: n.left + this.block.outerWidth() / 2 , y: n.top + this.block.outerHeight() / 2};
		} ,
		_getAngleValue: function (n , t) {
			var r = Math.atan2(n.y - t.y , t.x - n.x) , i = -r / (Math.PI / 180);
			return i < this._start && (i += 360) , i = this._checkAngle(i) , this._processStepByAngle(i);
		} ,
		_checkAngle: function (n) {
			var i = this._oriAngle(n) , t;
		...
		_limitAngle: function (n) {return n > 360 + this._start && (n -= 360) , n < this._start && (n += 360) , n;} ,
		_limitValue: function (n) {return n < this.options.min && (n = this.options.min) , n > this.options.max && (n = this.options.max) , n;} ,
		...
		_updateHidden: function () {this._hiddenField.val(this.options.value);} ,
		_updateTooltip: function () {
			this.tooltip && !this.tooltip.hasClass("hover") && this.tooltip.html(this._getTooltipValue());
			this._updateTooltipPos();
		} ,
		_updateTooltipPos: function () {this.tooltip && this.tooltip.css(this._getTooltipPos());} ,
		_getTooltipPos: function () {
			var n = this.options.circleShape , t;
		...
		_tooltipValue: function (n) {
			var t = this._raise("tooltipFormat" , {value: n , handle: this._handleArgs()});
		...
		_checkDataType: function () {
			var i = this.options , r , n , t , u = this._props();
			for (r in u.numberType) {
				n = u.numberType[r] , t = i[n] , i[n] = isNumber(t) ? parseFloat(t) : this.defaults[n];
			}
		...
		_analyzeModelValue: function () {
			var n = this.options.value , t = this.options.min , f = this.options.max , u , r ,
		...
		_validateModelValue: function () {
			...
			if (this._rangeSlider) {
				var u = r.split(",") , n = parseFloat(u[0]) , t = parseFloat(u[1]);
		...
		_isBrowserSupported: function () {
			for (var t = ["borderRadius" , "WebkitBorderRadius" , "MozBorderRadius" , "OBorderRadius" , "msBorderRadius" , "KhtmlBorderRadius"] , n = 0; n < t.length; n++) {
				if (document.body.style[t[n]] !== undefined) {
					return !0;
				}
			}
		} ,
		_throwError: function () {return "This browser doesn't support the border-radious property.";} ,
		_checkIE: function () {
		...
		_bind: function (n , t , i) {$(n).bind(t , $proxy(i , this));} ,
		_unbind: function (n , t , i) {$.proxy ? $(n).unbind(t , $.proxy(i , this)) : $(n).unbind(t);} ,
		_getInstance: function () {return $data(this.control[0] , pluginName);} ,
		...
		_get: function (n) {return this.options[n];} ,
		_set: function (n , t) {
			var i = this._props();
			if ($.inArray(n , i.numberType) != -1) {
				...
				$.inArray(n , i.booleanType) != -1 ? t = t == "false" ? !1 : !!t : $.inArray(n , i.stringType) != -1 && (t = t.toLowerCase());
			}
			if (this.options[n] != t) {
				this.options[n] = t;
				switch (n) {
			...
			if (n && isNumber(n)) {
				if (t && isNumber(t)) {
					if (this.options.sliderType == "range") {
						var i = parseFloat(t) , r = parseFloat(n);
		...
		destroy: function () {this._getInstance() && (this._destroyControl() , this._removeData() , this._originalObj.insertAfter(this.control) , this.control.remove());}
	};
	$.fn.rsRotate = function (n) {return setTransform(this , n);};
	typeof $.fn.outerHeight === "undefined" && ($.fn.outerHeight = function () {return this[0].offsetHeight;} , $.fn.outerWidth = function () {return this[0].offsetWidth;});
	typeof $.fn.hasClass === "undefined" && ($.fn.hasClass = function (n) {return this[0].className.split(" ").indexOf(n) !== -1;});
